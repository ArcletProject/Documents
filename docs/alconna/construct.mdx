...
- options (`Option`): 命令选项, 用于匹配命令选项, 详见 [命令选项](./node.mdx#Option)
...
其他参数的含义如下:
- action (`ArgAction | Callable | None`): 命令执行函数, 详见 [命令执行函数](./action.mdx)
- meta (`CommandMeta | None`): 命令元数据, 详见 [命令元数据](./meta.mdx)
- namespace (`str | Namespace | None`): 命令的命名空间, 详见 [命名空间](./config.mdx#Namespace)
- separators (`str | set[str] | Sequence[str] | None`): 命令分隔符, 详见 [命令分隔符](./node.mdx#separators)
- analyser_type (`type[TAnalyser] | None`): 命令分析器类型, 详见 [命令分析器](./analyser.mdx)
- behaviors (`list[ArparmaBehavior] | None`): 命令行参数行为, 详见 [命令行参数行为](./arparma.mdx#Behavior)
- formatter_type (`type[TextFormatter] | None`): 命令行参数格式化器类型, 详见 [命令行参数格式化器](./formatter.mdx)
